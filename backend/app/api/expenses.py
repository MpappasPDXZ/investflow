...
from app.services.document_service import upload_document, get_document_download_url
...
async def create_expense_with_receipt(
    ...
    try:
        ...
        expense_data = ExpenseCreate(
            ...
            document_storage_id=document["id"],
            is_planned=is_planned,
            notes=notes
        )
        
        expense_dict = create_expense(user_id=user_id, expense_data=expense_data)
        return ExpenseResponse(**expense_dict)
...
async def get_expense_endpoint(
    ...
    try:
        user_id = UUID(current_user["sub"])
        expense_dict = get_expense(expense_id, user_id)
...
async def update_expense_endpoint(
    ...
    try:
        user_id = UUID(current_user["sub"])
        expense_dict = update_expense(expense_id, user_id, expense_data)
...
async def get_expense_receipt(
    ...
    try:
        ...
        download_url = get_document_download_url(expense_dict["document_storage_id"], user_id)
