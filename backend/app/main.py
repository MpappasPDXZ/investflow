...
from app.api import auth, health, properties, users, units, scheduled, rent, expenses, documents, financial_performance, scheduled_template, comparables, leases, tenants, landlord_references, income_statement
...
async def startup_event():
    ...
    logger.info("Tabular store: Postgres-only (Lakekeeper removed)")
    ...
    try:
        ...
        if auth_cache.initialize():
            ...
            logger.warning("Auth cache initialization failed, will use Postgres/ADLS on first request")
    except Exception as e:
...
app.include_router(income_statement.router, prefix=settings.API_V1_PREFIX)
# Temporarily disabled
